...
namespace tek::interpreter {
        ...
        switch (expression.op.type) {
            ...
            case tokenizer::TokenType::BANG:
                return types::Literal(
                    !tek::interpreter::Interpreter::is_truthy(right));
        }
    ...
    Interpreter::visit_binary_expression(parser::BinaryExpression& expression) {
        ...
        switch (expression.op.type) {
            ...
            case tokenizer::TokenType::BANG_EQUAL: {
                return types::Literal(
                    !tek::interpreter::Interpreter::is_equal(left, right));
            }
    ...
    constexpr void Interpreter::assert_operand_types(const tokenizer::Token& op,
                                                     Variants&&... variants) {
        if ((variants::have_type_of<AssertType>(variants) && ...)) {
            return;
        }

        throw exceptions::RuntimeError(op,
                                       "Operand must be of type" +
                                           traits::TypeName<AssertType>::get());
    }

    std::string Interpreter::stringify(const types::Literal& value) {
        return value.str();
    }
