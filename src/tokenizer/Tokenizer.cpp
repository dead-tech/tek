...
Tokenizer::Tokenizer(const std::string& source_code) : source{source_code} {}
...
void Tokenizer::add_token(const TokenType type) { this->add_token(type, ""); }
    ...
    default:
        if (std::isdigit(c)) {
            ...
            Logger::error(this->line,
                          fmt::format("Unexepected character: {}", c));
        }
        break;
    }
}
